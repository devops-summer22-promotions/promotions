...
.PHONY: pwlogin
pwlogin: ## Login to IBM Cloud using username / password
	$(info Logging into IBM Cloud cluster $(CLUSTER)...)
	ibmcloud login -a cloud.ibm.com -g Default -r us-south
	ibmcloud cr login
	ibmcloud ks cluster config --cluster $(CLUSTER)
	kubectl cluster-info

.PHONY: team-login
team-login: ## Login to IBM Cloud team account (Alexander Lee)
	$(info Logging into IBM Cloud cluster $(CLUSTER)...)
